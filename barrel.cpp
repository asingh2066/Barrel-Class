...
namespace data_structures_assignment_1 {
    ...
    barrel::barrel(double init_height, double init_water_liters, double init_olive_oil_liters) {
        ...
        assert((height*100) >= (water_liters + olive_oil_liters));
    }
    ...
    void barrel::add_olive_oil(double liters){
        assert(liters > 0);
        if(liters + water_liters + olive_oil_liters > get_total_capacity()){
            olive_oil_liters = get_total_capacity() - water_liters;
        }
        else{
    ...
    void barrel::drain(double liters){
        ...
        else if(liters>water_liters){
            water_liters = 0;
            double change = liters - water_liters;
            olive_oil_liters = olive_oil_liters - change;
        }
        else if(liters < water_liters){
    ...
    bool operator==(const barrel &s1, const barrel &s2){
        if(s1.get_height() == s2.get_height() and
        s1.get_water_liters() == s2.get_water_liters() and
        s1.get_olive_oil_liters() == s2.get_olive_oil_liters())
            return true;
        else{
            return false;
